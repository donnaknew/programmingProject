...
$(document).ready(
    function () {
        //Get the camera list
        new PlatformServiceHelper().MakeServiceCall("webapp/GetDevicesList", "", GetDeviceListCallback);
        new PlatformServiceHelper().MakeServiceCall("webapp/GetCameraList", "", GetCameraListCallback);
        ShowDummyPortInfo();
        dummyCheck = setInterval(function () { ShowDummyPortInfo() }, 1000);
    }
);

function GetDeviceListCallback(context, result) {
    var a = '<p> - ';
    var b = '</p>';
    var checkbox = '  ON/OFF:<input type=checkbox id="';
    var checkbox2 = '" onClick=""/>'
    for (var i = 0; i < result.length; i++) {
        if (result[i][1] == "HUE") {
            $(deviceList).append(a + result[i][0] + ". " + result[i][1] + checkbox + result[i][1] + checkbox2 + b);
        } else {
            $(deviceList).append(a + result[i][0] + ". " + result[i][1] + b);
        }
    }
    for (var i = 0; i < result.length; i++) {
        if (result[i][1] == "HUE" && result[i][2] > 0) {
            var check = document.getElementById(result[i][1]);
            check.checked = true;
        }
    }
}
...
function SendImage() {
    ...
    new PlatformServiceHelper().MakeServiceCall("webapp/SendImage", '{"cameraFriendlyName": "' + g_currentCamera + '"}', SendImageCallback);
}

function SendImageCallback(context, result) {
...
function ShowDummyPortInfo() {
    new PlatformServiceHelper().MakeServiceCall("webapp/GetReceivedMessages", "", GetReceivedMessages);
}

function GetReceivedMessages(context, result) {
    $("#DummyList").html('');
    for (i = 0; i < result.length; i++) {
        $("#DummyList").append(result[i] + "&#13;&#10"); //replace <br /> with &#13;&#10 because that makes newlines in more browsers
    }
    $("input:text:").val([result[0]]);
}